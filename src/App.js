...
function App() {
  ...
  const [apiClient] = useState(
    () => new WiseFlowAPIClient('http://localhost:8080')
  );
  const [isBackendConnected, setIsBackendConnected] = useState(false);

  // PocketBase 客户端
  const [pbClient] = useState(
    () => new PocketBaseClient('http://localhost:8090')
  );
  ...
  const [keywords, setKeywords] = useState([]); // 保持字符串数组用于显示
  const [keywordObjects, setKeywordObjects] = useState([]); // 保存完整对象用于操作
  ...
  // 从后端加载数据 (已废弃 - 统一使用 PocketBase)
  const loadBackendData = async () => {
    console.log('loadBackendData 已废弃，统一使用 PocketBase 数据');
    // 如果 PocketBase 连接，直接使用 PocketBase 数据
    if (isPocketBaseConnected) {
      await loadPocketBaseData();
      return;
    }

    // 否则尝试使用 Python 后端作为备选
    try {
      addLog('尝试从 Python 后端加载数据...');

      // 加载信息源
      const sourcesResponse = await apiClient.getSources();
      if (sourcesResponse.sources) {
        setSources(sourcesResponse.sources);
        addLog(`从后端加载 ${sourcesResponse.sources.length} 个信息源`);
      }
      ...
      if (keywordsResponse.keywords) {
        ...
        const keywordObjs = keywordsResponse.keywords.map((k) => ({
          id: k.id || k.keyword, // 使用 keyword 作为 fallback ID
          keyword: k.keyword || k,
          activated: k.activated || true,
        }));
        setKeywordObjects(keywordObjs);
        setKeywords(keywordsResponse.keywords.map((k) => k.keyword || k));
        addLog(`从后端加载 ${keywordsResponse.keywords.length} 个关键词`);
      }
      ...
      if (infoResponse.info) {
        ...
        addLog(`从后端加载 ${infoResponse.info.length} 条信息`);
      }
    } catch (error) {
      console.error('加载后端数据失败:', error);
      addLog(`后端数据加载失败: ${error.message}`);
    }
  };
  ...
  const loadPocketBaseData = async () => {
    try {
      ...
      if (sourcesResponse.success && sourcesResponse.sources) {
        setSources(sourcesResponse.sources);
        addPocketbaseLog(
          `✅ 已加载 ${sourcesResponse.sources.length} 个信息源`
        );
      ...
      if (keywordsResponse.success && keywordsResponse.keywords) {
        setKeywordObjects(keywordsResponse.keywords); // 保存完整对象
        setKeywords(keywordsResponse.keywords.map((k) => k.keyword)); // 保存字符串用于显示
        addPocketbaseLog(
          `✅ 已加载 ${keywordsResponse.keywords.length} 个关注点`
        );
      ...
      if (hasPermissionIssues) {
        addPocketbaseLog(
          '🔑 解决方案: 访问 http://localhost:' +
            pocketbaseConfig.port +
            '/_/ 登录管理员账户'
        );
      }
    } catch (error) {
        ...
        if (config) {
          ...
          if (config.pocketbase) {
            setPocketbaseConfig((prev) => {
              ...
              if (
                newConfig.port &&
                newConfig.port !== pbClient.getCurrentPort()
              ) {
  ...
  const handleStartPocketBase = async () => {
    ...
    try {
      ...
      const result = await window.electronAPI.startPocketBaseService(
        pocketbaseConfig
      );
      if (result.success) {
        setPocketbaseStatus('running');
        ...
        if (keywordObj) {
          ...
          if (deleteResponse.success) {
            ...
            if (updatedResponse.success && updatedResponse.keywords) {
              setKeywordObjects(updatedResponse.keywords); // 更新完整对象
              setKeywords(updatedResponse.keywords.map((k) => k.keyword)); // 更新显示数组
          ...
          throw new Error('关键词对象未找到');
        }
      } else {
        // PocketBase 未连接时使用本地状态
        setKeywords(keywords.filter((k) => k !== keyword));
        setKeywordObjects(keywordObjects.filter((k) => k.keyword !== keyword));
  ...
  const toggleSource = async (id) => {
    try {
      if (isPocketBaseConnected) {
        // 使用 PocketBase - 注意：目前 PocketBase schema 中没有 enabled 字段
        // 这是一个前端显示状态，我们暂时保持本地状态管理
        setSources(
          sources.map((s) => (s.id === id ? { ...s, enabled: !s.enabled } : s))
        );
        addPocketbaseLog(
          `切换信息源状态: ${sources.find((s) => s.id === id)?.name}`
        );
      } else {
        // PocketBase 未连接时使用本地状态
        setSources(
        ...
        addPocketbaseLog(
          `本地切换信息源状态: ${sources.find((s) => s.id === id)?.name}`
        );
      }
    } catch (error) {
      console.error('切换信息源状态失败:', error);
      addPocketbaseLog(`切换信息源状态失败: ${error.message}`);
      // 失败时回退到本地状态
      setSources(
        sources.map((s) => (s.id === id ? { ...s, enabled: !s.enabled } : s))
      );
        ...
        <div className="flex items-center space-x-4">
            ...
            <p className="text-sm text-gray-600 mb-2">
              ...
              <span
                className={`font-medium ${
                  isBackendConnected ? 'text-green-600' : 'text-red-600'
                }`}
              >
          ...
          <div className="space-x-2">
            ...
            {serviceStatus === 'running' && !isBackendConnected && (
              <button
                onClick={
                  connectionPolling ? stopConnectionPolling : handleReconnectAPI
                }
              ...
              >
                <RefreshCw
                  className={`w-4 h-4 mr-2 ${
                    isReconnecting || connectionPolling ? 'animate-spin' : ''
                  }`}
                />
                {connectionPolling
        ...
        <div className="flex items-center space-x-4">
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600 mb-2">
              ...
              <span
                className={`font-medium ${
                  isPocketBaseConnected ? 'text-green-600' : 'text-red-600'
                }`}
              >
  ...
  const renderKeywords = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        {isPocketBaseConnected && keywordObjects.length > 0 ? (
          // PocketBase 连接时显示详细信息
          <div className="divide-y divide-gray-200">
            {keywordObjects.map((keywordObj) => (
              <div key={keywordObj.id} className="px-6 py-4">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3">
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${
                          keywordObj.activated
                            ? 'bg-green-100 text-green-800'
                            : 'bg-gray-100 text-gray-600'
                        }`}
                      >
                        {keywordObj.activated ? '已启用' : '已禁用'}
                      </span>
                    </div>
                    {keywordObj.freq && (
                      <p className="text-sm text-gray-600 mt-1">
                        监控频率: 每 {keywordObj.freq} 小时
                      </p>
                    )}
                    {keywordObj.explanation && (
                      <p className="text-sm text-gray-600 mt-1">
                        说明: {keywordObj.explanation}
                      </p>
                    )}
                    {keywordObj.restrictions && (
                      <p className="text-sm text-gray-500 mt-1">
                        限制: {keywordObj.restrictions}
                      </p>
                    )}
                  </div>
                  <button
                    onClick={() => removeKeyword(keywordObj.keyword)}
        ...
        {keywords.length === 0 && (
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1">添加关键词以开始监控</p>
          </div>
        )}
      </div>
    </div>
        ...
        {ksCache.map((item) => (
          <div key={item.id} className="bg-white rounded-lg shadow-md p-4">
            ...
            <div className="space-y-2 text-xs text-gray-500">
              ...
              {item.create_time && (
                <div className="text-xs text-gray-400">{item.create_time}</div>
              )}
            </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              PocketBase 管理界面将在 http://localhost:{pocketbaseConfig.port}
              /_/ 可用
            </p>
          </div>
        </div>
